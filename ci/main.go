...
func main() {
	containerName := "ca-app-dagger-001"
	containerGroupName := "cg-app-dagger-001"
	containerGroupLocation := "westeurope"
	resourceGroupName := "rg-dagger-shared-001"
	...
	node := daggerClient.Container(dagger.ContainerOpts{
		Platform: "linux/amd64",
	}).From("node:18")

	ctr := node.WithDirectory("/src", source).WithWorkdir("/src").WithExec([]string{"cp", "-R", ".", "/home/node"}).WithWorkdir("/home/node").WithExec([]string{"npm", "install"}).WithEntrypoint([]string{"npm", "start"})

	dockerHubUsername := os.Getenv("DOCKERHUB_USERNAME")
	addr, err := ctr.WithRegistryAuth("docker.io", dockerHubUsername, dockerHubPassword).Publish(ctx, fmt.Sprintf("%s/ca-app-dagger-001", dockerHubUsername))
